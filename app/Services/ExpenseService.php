...
class ExpenseService
{
    public function getByMonth($user_id)
    {
        ...
        $expenses = Expense::whereYear('due_date', $year)->whereMonth('due_date', $month)->where('user_id', $user_id)->get();
    ...
    public function getByType($user_id)
    {
        ...
        $expensesQuery = Expense::join('expense_types', 'expenses.expense_type_id', '=', 'expense_types.id')->selectRaw('expense_types.name AS name, SUM(expenses.cost) AS cost, expense_types.color as color')->where('expenses.user_id', $user_id)->groupBy('expense_types.id', 'expense_types.name', 'expense_types.color');

        // Separate query: Sum of costs from the unrelated table
        $additionalCostsQuery = RecurrentExpenses::where('user_id', $user_id)->selectRaw('"Recurrent expenses" AS name, SUM(cost) AS cost, "green" AS color');

        // Combine using union
        $expenses = $expensesQuery
            ->union($additionalCostsQuery)
            ->get()
    ...
    {
        return Expense::with(['expenseType', 'paymentMethod'])
            ->get()
    ...
    {
        $expenses = Expense::with(['expenseType', 'paymentMethod'])
            ->get()
            ->where('user_id', $user_id);
        return $expenses;
    }
